---
name: canvas-prompt-workbuddy-read
description: Read and continue from the latest exported Canvas Prompt round in the active scope. The canonical continuation commands are "根据画布内容推进" and "Continue with the canvas context"; invoke this skill whenever the user uses either command, then read the latest package before responding. Treat legacy "根据白板内容推进" as a compatible alias. Also use when the user explicitly asks WorkBuddy to read, understand, or continue from an exported Canvas Prompt round. Do not activate from an unrelated vague prompt or merely because an old project archive exists.
---
...
1. Use the `canvas_prompt` MCP server's `get_latest_prompt_package` tool without a project path or conversation token. It reads the user's single active board. If it reports no round, say that the board has no exported round yet and do not inspect a live browser canvas as a substitute. Prefer the returned Compact Package path, then Process IR; the raw package is for evidence checks. Inline screenshots and keyframes are intentionally excluded from normal MCP text to protect context budget. If visual inspection is necessary, open one returned local PNG path rather than requesting Base64. **If the MCP server is not registered as a tool in the current host session** (WorkBuddy loads MCP servers at session start from `.mcp.json` at the workspace root; a server placed in a subdirectory is not picked up mid-session), use the maintained CLI fallback: `node bin/canvas-prompt.mjs read` — this command imports and calls the exact same `handleGetLatestPromptPackage` function from `mcp/server.mjs` and returns identical JSON. It is a first-class CLI command, NOT a temporary Bash/Python read of the JSON file. Do not read `~/.canvas-prompt/board/latest-prompt-package.json` directly with Bash, Python, `cat`, or any ad-hoc script.
...
4. Route by the user's actual objective, without exposing a mode choice:
   - **Change path.** When an imported image, existing work, or a prior canvas structure is being marked with circles, crosses, arrows, replacements, or explicit change language, compile the final snapshot, marks, spatial relations, and linked speech into a visual edit brief.
     ...
     - In WorkBuddy, use Plan mode when it is available. If the host does not expose a Plan mode, produce the same confirmation plan in chat: "我理解的修改／准备怎么做／待确认" (or the equivalent in the user's language). Do not claim to have switched UI modes when the host does not expose one.
...
- Read the Compact Package before any browser or image tool. Its role is to keep the first interpretation bounded even in a long WorkBuddy task.
...
- WorkBuddy does not provide a native thread-injection protocol. The round's `handoff.json` records `host: 'workbuddy'` with `status: 'archived'` when the round is saved locally. Say "saved locally" when the archive exists. Do not claim "delivered" or "accepted" since WorkBuddy does not perform native thread injection. The durable contract is: the round is saved locally and available for the AI to read through the `canvas_prompt` MCP server on explicit request.
- Keep the response in the WorkBuddy conversation unless the user explicitly asks for an AI addition to the canvas.
