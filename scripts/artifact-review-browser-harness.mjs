...
async function main() {
  ...
  try {
    ...
    if (scenario === 'entry') {
      ...
      prototypePage.on('console', (message) => { if (message.type() === 'error') prototypeErrors.push(`console: ${message.text()}`); });
      await prototypePage.goto(`${origin}/interaction-review-i0/index.html`);
      await prototypePage.getByRole('link', { name: '返回交互审阅' }).waitFor({ state: 'visible', timeout: 15_000 });
      await prototypePage.getByRole('button', { name: '开始体验' }).click();
      await prototypePage.getByText('正在记录本轮体验。', { exact: true }).waitFor({ state: 'visible', timeout: 15_000 });
      if (await prototypePage.locator('[data-agent-lineage-receipt]').count() !== 1) throw new Error('Prototype session lineage receipt is missing.');
      await prototypePage.getByRole('button', { name: '让 AI 开始演示' }).click();
      await prototypePage.getByText('让 AI 走一遍 · 演示完成', { exact: true }).waitFor({ state: 'visible', timeout: 15_000 });
      const prototypeFeedback = prototypePage.getByPlaceholder('例如：点击生成后不知道系统在做什么，希望显示进度。');
      await prototypeFeedback.fill('两个版本的差异不够明显。');
      await prototypePage.getByRole('button', { name: '记录这条反馈' }).click();
      await prototypePage.getByText('关联位置：第 3 步 · 比较结果', { exact: true }).waitFor({ state: 'visible', timeout: 15_000 });
      if (await prototypeFeedback.inputValue() !== '') throw new Error('Prototype feedback input was not cleared after capture.');
      await prototypePage.getByRole('button', { name: '结束审阅' }).click();
      await prototypePage.getByText('本轮体验已结束，反馈已保留在当前页面。', { exact: true }).waitFor({ state: 'visible', timeout: 15_000 });
