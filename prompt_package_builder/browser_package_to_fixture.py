...
REJECTION_ASSOCIATION_WINDOW_MS = 6_000
...
def normalized_captions(package):
    ...
    if not isinstance(transcript, dict) or transcript.get("alignment_status") == "unavailable":
        return []
    ...
    for index, segment in enumerate(segments, start=1):
        if not isinstance(segment, dict) or not str(segment.get("text", "")).strip():
            continue
        # The browser segment ID is a cross-layer evidence key. Preserve it;
        # generating cap_* IDs here used to leave review evidence pointing at
        # foreign seg_* IDs that did not exist in Process IR.
        caption_id = str(segment.get("segment_id") or segment.get("caption_id") or f"cap_{index:03d}").strip()
        if not caption_id or caption_id in seen_ids:
            caption_id = f"cap_{index:03d}"
            while caption_id in seen_ids:
                caption_id = f"{caption_id}_dup"
        ...
        captions.append({
            ...
            "speaker": segment.get("speaker") or "user",
        })
    return captions
...
RED_MARKER_COLORS = {
    # Excalidraw's current palette values. Keep named colours for older exports,
    # but do not rely on the literal word "red": browser packages normally
    # carry hex values such as #dc2626.
    "#dc2626", "#ef4444", "#e03131", "#f03e3e",
}


def _is_red_marker_color(value):
    color = str(value or "").strip().lower()
    if "red" in color or color in RED_MARKER_COLORS:
        return True
    ...
    try:
        ...
        return False
    # Broad enough for red ink, narrow enough not to treat orange or purple as
    # a correction mark.
    return red >= 150 and red >= green * 2.5 and red >= blue * 1.45


def red_markers(objects):
    return [
        obj for obj in objects
        if _is_red_marker_color((obj.get("properties") or {}).get("color"))
    ]
...
def geometric_marker_candidates(objects, marker, max_gap_px=18):
    """Return inspectable target candidates ordered by geometric support."""
    ...
    return sorted(candidates, key=lambda item: (item[0], -item[1].get("timestamp_ms", 0), item[1].get("object_id", "")))


def geometric_marker_target(objects, marker, max_gap_px=18):
    candidates = geometric_marker_candidates(objects, marker, max_gap_px)
    if not candidates or (len(candidates) > 1 and abs(candidates[0][0] - candidates[1][0]) < 1.0):
        return None
    return candidates[0][1]


def _explicit_labels(obj):
    labels = []
    semantic_content = obj.get("semantic_content")
    if isinstance(semantic_content, str) and semantic_content.strip():
        labels.append(semantic_content.strip())
    props = obj.get("properties") or {}
    labels.extend(
        value.strip()
        for key in ("text", "label", "title", "content", "name")
        if isinstance((value := props.get(key)), str) and value.strip()
    )
...
def object_has_explicit_label(obj):
    ...
    """
    return bool(_explicit_labels(obj))


def _distance_to_time_range(timestamp_ms, start_ms, end_ms):
...
def build_rejection_links(negation_caps, markers, objects, events):
    """Join speech, marker, target, and exact event only within one episode."""
    candidates = []
    for caption in negation_caps:
        for marker in markers:
            marker_time = int(marker.get("timestamp_ms", 0) or 0)
            temporal_distance = _distance_to_time_range(marker_time, int(caption["start_ms"]), int(caption["end_ms"]))
            if temporal_distance > REJECTION_ASSOCIATION_WINDOW_MS:
                continue
            targets = geometric_marker_candidates(objects, marker)
            if not targets or (len(targets) > 1 and abs(targets[0][0] - targets[1][0]) < 1.0):
                continue
            marker_events = [
                event for event in events
                if event.get("object_id") == marker.get("object_id")
                and int(event.get("timestamp_ms", 0) or 0) == marker_time
                and event.get("event_type") != "pause"
            ]
            if len(marker_events) != 1:
                continue
            gap, target = targets[0]
            candidates.append({
                "caption_id": caption["caption_id"],
                "marker_object_id": marker["object_id"],
                "marker_event_id": marker_events[0]["event_id"],
                "target_object_id": target["object_id"],
                "marker_timestamp_ms": marker_time,
                "temporal_distance_ms": temporal_distance,
                "geometry_gap_px": round(gap, 2),
                "resolution_status": "supported",
            })
    candidates.sort(key=lambda item: (item["temporal_distance_ms"], item["geometry_gap_px"], item["caption_id"], item["marker_object_id"]))
    used_captions = set()
    used_markers = set()
    links = []
    for candidate in candidates:
        if candidate["caption_id"] in used_captions or candidate["marker_object_id"] in used_markers:
            continue
        used_captions.add(candidate["caption_id"])
        used_markers.add(candidate["marker_object_id"])
        links.append(candidate)
    return links


def object_states(captions, objects, negation_caps, confirmation_caps, selection_caps, markers, versions, rejection_links=None):
    # Object creation is a lifecycle observation, not evidence that the object
    # is a candidate in the person's reasoning. Only emit a reasoning-state
    # transition when explicit language can be bound to a named object.
    states = []

    def append_state(target, state_to, timestamp_ms, reason, captions_used, source_event_ids=None):
        states.append({
            ...
            "source_caption_ids": [caption["caption_id"] for caption in captions_used],
            "assertion_level": "assertion",
            "evidence": [{"type": "speech_anchor", "caption_id": caption["caption_id"]} for caption in captions_used],
            "counter_evidence": [],
        })

    caption_by_id = {caption["caption_id"]: caption for caption in captions}
    object_by_id = {obj.get("object_id"): obj for obj in objects}
    for link in rejection_links or []:
        caption = caption_by_id.get(link["caption_id"])
        target = object_by_id.get(link["target_object_id"])
        if caption and target:
            append_state(
                target,
                "rejected",
                link["marker_timestamp_ms"],
                "supported_negation_marker_link",
                [caption],
                [link["marker_event_id"]],
            )
    ...
    if promotion_caps and len(versions) < 3:
        ...
        if target:
            append_state(target, "promoted", confirmation["start_ms"], "confirmation_text", [confirmation])
    ...
    if len(versions) >= 3:
        ...
        for (mention, target), (next_mention, _) in zip(version_objects, version_objects[1:]):
            append_state(target, "superseded", next_mention["timestamp_ms"], "later_version_refinement", [caption_by_id[mention["caption_id"]], caption_by_id[next_mention["caption_id"]]])
        if version_objects:
            mention, target = version_objects[-1]
            append_state(target, "promoted", (confirmation_caps[-1]["start_ms"] if confirmation_caps else mention["timestamp_ms"]), "final_version_confirmation", [caption_by_id[mention["caption_id"]]])
    return states


def semantic_events(captions, events, objects, negation_caps, confirmation_caps, selection_caps, reframe_caps, markers, versions, rejection_links=None):
    generated = []
    ...
    caption_by_id = {caption["caption_id"]: caption for caption in captions}
    for index, link in enumerate(rejection_links or [], start=1):
        caption = caption_by_id.get(link["caption_id"])
        if not caption:
            continue
        generated.append(
            {
                "semantic_event_id": f"sem_reject_{index:03d}",
                "type": "reject_path",
                "time_range_ms": [min(caption["start_ms"], link["marker_timestamp_ms"]), max(caption["end_ms"], link["marker_timestamp_ms"])],
                "summary": "用户明确否定先前表述，并在同一时间窗用视觉标记回看该路径。",
                "source_caption_ids": [link["caption_id"]],
                "source_canvas_event_ids": [link["marker_event_id"]],
                "involved_object_ids": [link["target_object_id"]],
                "rejection_link": link,
                "confidence": 0.8,
            }
        )
...
def _candidate_features(event, evidence):
    ...
    rejection_link = event.get("rejection_link") if isinstance(event.get("rejection_link"), dict) else {}
    return {
        ...
        "temporal_distance_ms": rejection_link.get("temporal_distance_ms"),
        "marker_target_resolution": rejection_link.get("resolution_status"),
    }
...
def _calibrate_candidate(event, features):
    ...
    if event_type == "reject_path":
        supported = (
            isinstance(features.get("temporal_distance_ms"), (int, float))
            and features["temporal_distance_ms"] <= REJECTION_ASSOCIATION_WINDOW_MS
            and features.get("marker_target_resolution") == "supported"
            and features["canvas_action_ref_count"] == 1
            and features["named_object_ref_count"] == 1
        )
        if not supported:
            counter_evidence.append({
                "kind": "unsupported_rejection_association",
                "summary": "否定语音、视觉标记与具名目标没有形成同一时间窗内的唯一证据链。",
            })
            return round(min(confidence, 0.6), 2), "hypothesis", counter_evidence
...
def adapt(package):
    ...
    rejection_links = build_rejection_links(negation_caps, markers, objects, events)
    candidates = enrich_candidates(
        semantic_events(captions, events, objects, negation_caps, confirmation_caps, selection_caps, reframe_caps, markers, versions, rejection_links),
        captions,
        objects,
    )
    ...
    states = object_states(captions, objects, negation_caps, confirmation_caps, selection_caps, markers, versions, rejection_links)
    fusion = [
        {
    ...
    snapshot = package.get("canvas_snapshot") or {}
    state_frames = []
    for index, frame in enumerate(snapshot.get("keyframes") or [], start=1):
        if not isinstance(frame, dict) or not isinstance(frame.get("image"), dict):
            continue
        image = frame["image"]
        state_frames.append({
            "frame_id": f"state_frame_{index:03d}",
            "timestamp_ms": max(0, int(frame.get("timestamp_ms", 0) or 0)),
            # The image blob remains in the local round archive. The compact
            # package only carries an inspectable reference, never duplicate
            # base64 image bytes.
            "image": {
            ...
            "constraint": "state_frame_is_visual_evidence_not_semantic_interpretation",
        })
    return {
        ...
        "final_canvas_snapshot": snapshot.get("final", {}),
        "state_frame_track": state_frames,
...
def save_fixture(adapted, fixture_dir: Path):
    ...
    write_json(fixture_dir / "state_frame_track.json", {"state_frames": adapted["state_frame_track"]})
