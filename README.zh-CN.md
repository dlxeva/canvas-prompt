...
**第一波公开发布以 Codex Desktop 为已验证、推荐的集成。** Canvas Prompt 以本地画布打开，导出一轮不可变上下文后由 Codex 通过 MCP 读取。项目决定档案放在哪里；宿主明确提供的当前对话 ID 才决定它属于哪段对话。其他支持 MCP 的终端可以读取本地包，但只是可选兼容路径，不承诺同等的原生界面或交接体验。
