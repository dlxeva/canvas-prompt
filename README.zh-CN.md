...
**第一波公开发布以 Codex Desktop 为已验证、推荐的集成。** Canvas Prompt 以本地画布打开并导出一轮不可变上下文。当前 Codex Desktop 集成请在同一任务输入固定续接指令 **“根据画布内容推进”**，随后由 Codex 经 MCP 读取最新包。项目决定档案放在哪里；宿主明确提供的当前对话 ID 才决定它属于哪段对话。其他支持 MCP 的终端可以读取本地包，但只是可选兼容路径，不承诺同等的原生界面或交接体验。
...
- **图片批阅**：把图片放到画布上，圈出区域并说出修改要求；最终快照、标记和语音会组成 visual brief。快照清晰时可直接生成新版，细节不足时只需在主对话补充原图。Codex 在主对话生成新版图片后，可复制图片并回到画布按 `⌘V` 粘贴，作为下一轮批阅的底图。
