...
**第一波公开发布以 Codex Desktop 为已验证、推荐的集成。** Canvas Prompt 以本地画布打开并导出一轮不可变上下文。当前 Codex Desktop 集成请在同一任务输入固定续接指令 **“根据画布（白板）内容推进”**，随后由 Codex 经 MCP 读取最新包。项目决定档案放在哪里；宿主明确提供的当前对话 ID 才决定它属于哪段对话。其他支持 MCP 的终端可以读取本地包，但只是可选兼容路径，不承诺同等的原生界面或交接体验。
...
安装或更新插件后，请新建一个 Codex 任务，让最新的 Skill 与 MCP 工具重新载入。结束一轮后，请在同一任务输入固定续接指令 **“根据画布（白板）内容推进”**。这比伪造一个 Desktop 尚未公开给插件的自动线程桥接更诚实、也更稳定。
