...
**第一波公开发布以 Codex Desktop 为已验证、推荐的集成。** Canvas Prompt 以本地画布打开并导出一轮不可变上下文。在 Codex 对插件 Hook 完成一次信任后，同一任务里的下一句普通追问会收到这一轮的有界续接上下文；MCP 仍用于显式读取和后续深读。项目决定档案放在哪里；宿主明确提供的当前对话 ID 才决定它属于哪段对话。其他支持 MCP 的终端可以读取本地包，但只是可选兼容路径，不承诺同等的原生界面或交接体验。
...
- **对话作用域档案**：宿主提供当前对话 ID 时，每一轮会落入独立的对话作用域；有项目时在 `.canvas-prompt/threads/<不透明键>/` 下，无项目时在用户私有目录下。原始对话 ID 不会用作文件路径。
- **仅显式交接**：Canvas Prompt 先保存不可变上下文包。它绝不根据项目最近记录、工作区历史或上次绑定选择对话；若宿主无法提供当前对话 ID，仍可显式读取本地包，但不能声称已自动路由到用户眼前的主对话。
...
v0.1 对 Codex Desktop 承诺：可靠保存一次白板推演，并由已获得当前对话 ID 的宿主在同一对话作用域读取该包。当前宿主若无法提供这一 ID，只能使用显式项目读取，不能宣称等价的当前对话交接。其他 MCP 宿主同理，属于待验证的可选兼容路径。圈选、箭头、鼠标停留和中英文指代目前都只作为观察或候选证据，不会自动写成确定语义。
...
| AI 继续对话 | 对话作用域 MCP | 宿主提供当前对话 ID 时，读写固定在该作用域；没有 ID 时只能显式读取项目内档案，不得暗示已自动续接可见对话。 |
...
安装或更新插件后，请新建一个 Codex 任务，让最新的 Skill 与 MCP 工具重新载入。Codex 会列出插件自带的 `UserPromptSubmit` Hook 供审核；请一次性信任 Canvas Prompt Hook。它只读取当前提示词/会话元数据，并在该会话完成一轮画布后，把一份有界的本地续接摘要附加到下一次用户输入；不会上传画布，也不会替用户选择另一段对话。
...
`init` 会输出固定作用域的 MCP 配置：带 `--thread-id` 时为对话作用域，不带时为显式项目作用域。画布完成本地保存与编译后，其他宿主可以经 MCP 读取该作用域的 Canvas Prompt Package。v0.1 不声称任何宿主具有原生侧边栏、自动把内容写入当前对话或自动续接的能力。
