...
**通过条件：** 轮次包含图片素材和相对素材的批注观察；固定续接指令后，主对话把最终快照、标记和语音组成 visual brief。快照清晰时直接用于生成干净的新版本，不因缺少原文件路径而停止；快照不足时只要求测试者在当前对话补充原图，不要求重复标注。未口述的圈保持未决，不被说成已确认的编辑意图；视图缩放/平移不被改写成对象移动语义。

## Round 2b：新版图片回流（项目 A）
