...
这份运行单只验证 v0.1 已承诺的主链路：一次白板过程被保存到正确项目，并能在同一 Codex 任务的下一句自然追问中接续；MCP 仍可读取该不可变产物用于显式和后续深读。
...
- 在 Codex 的 Hook 审核中一次性信任插件随附的 Canvas Prompt `UserPromptSubmit` Hook；未信任时，Round 1 的自然续接不具备验收资格。
...
4. 在当前主对话只输入“你怎么看？”（或 “what do you think?”），不要提示它“读取画布”或粘贴 Package ID；确认这条普通追问自动收到本会话刚结束轮次的续接上下文，并据此继续讨论。

**通过条件：** 包含画笔记录和完整编译产物；主对话把这句自然追问识别为刚结束白板轮次的续接，并且没有要求测试者手动粘贴 JSON、说出读取指令或切换到别的项目。只有当该任务自身已经打开并结束过画布时，这一自动续接才应触发；它只消费一次，不应污染后续无关对话。
