...
这份运行单只验证 v0.1 已承诺的主链路：一次白板过程被保存到正确项目，并能由当前宿主经项目绑定 MCP 读取。
...
4. 在当前主对话只输入“你怎么看？”（或 “what do you think?”），不要提示它“读取画布”或粘贴 Package ID；确认它通过 Canvas Prompt MCP 读取这轮上下文后能继续讨论。

**通过条件：** 包含画笔记录和完整编译产物；主对话把这句自然追问识别为刚结束白板轮次的续接，并且没有要求测试者手动粘贴 JSON、说出读取指令或切换到别的项目。只有当该任务自身已经打开并结束过画布时，这一自动读取才应触发。
