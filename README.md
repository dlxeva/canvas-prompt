...
- **Image review rounds**: place an image on the canvas, mark regions, and explain requested changes aloud. The final snapshot, marks, and speech form a visual brief; use a clear snapshot directly, or attach the original image in chat when more fidelity is needed. When Codex produces a revised image, copy it from the conversation and press `⌘V` on the canvas to use it as the substrate for the next review round.
